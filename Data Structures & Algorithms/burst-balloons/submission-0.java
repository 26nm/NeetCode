class Solution {
    public int maxCoins(int[] nums) {
        ...
        // create array to hold balloons
        int[] balloons = new int[n + 2];

        // set the 1st and last elements to 1
        balloons[0] = 1;
        balloons[n + 1] = 1;
        ...
        // create 2d dp array of size (nx2) x (nx2)
        ...
        for (int len = 1; len <= n; len++) {
            ...
            for (int left = 1; left <= n - len + 1; left++) {
                ...
                for (int last = left; last <= right; last++) {
                    // compute # of coins from left interval, right interval,
                    // and bursting last balloon
                    int coins = balloons[left - 1] * balloons[last] * balloons[right + 1]
                        + dp[left][last - 1] + dp[last + 1][right];

                    // update left & right in dp to whatever is bigger among
                    // current value and coins computed
