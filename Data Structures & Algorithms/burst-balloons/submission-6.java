...
*
* ith element represents a balloon with integer value nums[i]
* -you must burst all balloons
*
* if you burst ith balloon, you will receive
* nums[i-1] x nums[i] x nums[i+1] coins
* -if (i-1) or (i+1) goes out of bounds, assume out-of-bounds
*  value is 1
*
...
class Solution {
    public int maxCoins(int[] nums) {
        ...
        // create int array of size n+2 to hold balloons
        ...
        // iterate from 1 to n for each interval
        for(int len = 1; len <= n; len++) {
            ...
            for(int left = 1; left <= n - len + 1; left++) {
                ...
                for(int last = left; last <= right; last++) {
                    // compute # of coins from bursting balloons from
                        // left, right, and last intervals
        ...
        // return results for entire interval
        return dp[1][n];
    }
}
